...
const HOSPITAL2_SERVER = 'http://localhost:3002';
...
app.get('/get-patient/:id', async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        try {
            const response = await axios.get(`${HOSPITAL2_SERVER}/get-patient/${patientID}`);
            res.status(200).json(response.data);
        } catch (error) {
...
const PORT = 3001;
app.listen(PORT, () => {
    console.log(`Hospital 1 Server is running on port ${PORT}`);
});
